...
export async function payForImageGeneration(wallet, connection) {
  ...
  try {
    ...
    console.log("PAYMENT: Getting latest blockhash...");
    let blockhash, lastValidBlockHeight;
    try {
      ({ blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash('confirmed'));
      console.log("PAYMENT: Successfully fetched blockhash:", blockhash);
    } catch (rpcError) {
      console.error("PAYMENT: Failed to get blockhash:", rpcError);
    ...
    try {
      ...
      console.log(`PAYMENT: Attempting to confirm transaction ${signature}...`);
      const confirmation = await connection.confirmTransaction({
        signature,
        blockhash,
        lastValidBlockHeight
      }, 'confirmed');
      console.log(`PAYMENT: Transaction ${signature} confirmed. Result:`, confirmation);
      ...
      if (signSendConfirmError instanceof Error && signSendConfirmError.message.includes('confirmTransaction')) {
         throw new Error(`Payment confirmation timeout or error: ${signSendConfirmError.message}`);
      } else if (signature) {
