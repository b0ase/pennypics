...
export async function buyTokens(connection, buyer) {
  try {
    // Always use fixed values to prevent calculation errors
    const tokenAmount = 10; // Always buy exactly 10 tokens
    const solCost = 0.01; // Fixed cost: 0.001 SOL per token × 10 tokens = 0.01 SOL
    
    console.log(`TOKEN_BUY: Buying ${tokenAmount} tokens for ${solCost} SOL`);
    
    // Ensure lamports is a valid integer
    const lamports = 10000000; // Exactly 0.01 SOL in lamports (1 SOL = 1,000,000,000 lamports)
