...
export async function createPennyPicsToken(connection, payer) {
  try {
    ...
    // Debug the wallet contents
    console.log("TOKEN_CREATE: Wallet object:", {
      publicKeyExists: !!payer.publicKey,
      publicKeyType: payer.publicKey ? typeof payer.publicKey : 'undefined',
      publicKeyPrototype: payer.publicKey ? Object.getPrototypeOf(payer.publicKey) : 'undefined',
      hasSignTransaction: typeof payer.signTransaction === 'function',
      hasSignAllTransactions: typeof payer.signAllTransactions === 'function',
      walletName: payer.wallet?.adapter?.name || 'unknown'
    });
    ...
    console.log("TOKEN_CREATE: Payer public key constructor:", payerPublicKey.constructor.name);
    console.log("TOKEN_CREATE: Payer public key instanceof PublicKey:", payerPublicKey instanceof PublicKey);
    console.log("TOKEN_CREATE: Creating token mint with payer:", payerPublicKey.toString());
    
    // Ensure publicKey is a proper Solana PublicKey object
    const verifiedPayerPublicKey = payerPublicKey instanceof PublicKey 
      ? payerPublicKey 
      : new PublicKey(payerPublicKey.toString());
    ...
    const mint = await createMint(
      ...
      verifiedPayerPublicKey, // Mint authority - using admin's wallet for simplicity
