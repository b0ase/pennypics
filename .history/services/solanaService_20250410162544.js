...
export async function checkBalance(wallet, connection) {
  ...
  try {
    // Direct balance check with retries
    let balance = null;
    let error = null;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        console.error(`Balance check attempt ${attempt + 1} failed:`, err);
        error = err;
    ...
    if (balance === null) {
      throw error || new Error('Failed to get balance after multiple attempts');
    }
