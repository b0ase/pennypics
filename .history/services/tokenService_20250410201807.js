...
// DISABLED FOR NOW to avoid browser issues
/*
...
// Cost for 1 token/credit in SOL - ensure it's a valid number
// Using a hard-coded value to prevent NaN errors
export const TOKEN_COST_SOL = 0.001; // Fixed value in SOL

// Number of tokens per purchase
export const TOKENS_PER_PURCHASE = 10; // Fixed number of tokens
...
export async function buyTokens(connection, buyer) {
  try {
    // Standard amount in tokens - exactly 10 PENNY tokens per purchase
    const tokenAmount = TOKENS_PER_PURCHASE; // 10 tokens
    
    // Price in SOL - ensure it's a valid number
    const solCost = parseFloat(TOKEN_COST_SOL) * tokenAmount;
    if (isNaN(solCost) || solCost <= 0) {
      throw new Error("Invalid token cost calculation");
    }
    ...
    // Ensure lamports is a valid integer
    const lamports = Math.floor(solCost * LAMPORTS_PER_SOL);
    if (isNaN(lamports) || lamports <= 0) {
      throw new Error(`Invalid lamports amount: ${lamports}. Please check TOKEN_COST_SOL value.`);
    }
    ...
    // Instead of trying to transfer from treasury (which would require server implementation),
    // we'll just update the local storage with exactly 10 more tokens
    
    // Return success information
    return {
      success: true,
      paymentSignature,
      tokenTransferPending: false, // No longer pending as we're handling it here
      message: "Payment successful! 10 PENNY tokens have been added to your account.",
      tokenAmount: 10, // Exactly 10 tokens
      solCost
    };
  } catch (error) {
...
export function spendTokensForImage() {
  ...
  // Always deduct exactly 1 token
  const newBalance = numTokens - 1;
  localStorage.setItem('pennypics_image_credits', newBalance.toString());
  
  console.log(`TOKEN_SPEND: Spent 1 token. Balance was ${numTokens}, now ${newBalance}`);
