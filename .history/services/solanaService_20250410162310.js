...
export async function checkBalance(wallet, connection) {
  try {
    ...
    while (attempts < maxAttempts && balance === null) {
      try {
        // Get the balance with confirmed commitment
    ...
    // Try one more time with a new connection to mainnet
    try {
      console.log("Retrying balance check with new mainnet connection...");
      const newConnection = new Connection('https://api.mainnet-beta.solana.com', {
        commitment: 'confirmed'
      });
      
      const balance = await newConnection.getBalance(wallet.publicKey, 'confirmed');
      const requiredLamports = Math.floor(IMAGE_COST * LAMPORTS_PER_SOL);
      const hasEnough = balance >= requiredLamports;
      
      console.log(`Retry balance check: ${balance / LAMPORTS_PER_SOL} SOL`);
      return hasEnough;
    } catch (retryError) {
