...
export async function payForImageGeneration(wallet, connection) {
  ...
  try {
    ...
    try {
      // Use 'finalized' for better reliability against expiration
      ({ blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash('finalized')); 
      console.log("PAYMENT: Successfully fetched finalized blockhash:", blockhash);
    } catch (rpcError) {
    ...
    try {
      ...
      // Confirm the transaction using 'finalized' commitment
      console.log(`PAYMENT: Attempting to confirm transaction ${signature} with 'finalized' commitment (up to 90s)...`);
      const confirmation = await connection.confirmTransaction({
        signature,
        blockhash,
        lastValidBlockHeight,
        // Explicitly set a longer timeout for this specific confirmation call
        commitment: 'finalized', // Redundant but ensures clarity
        confirmTransactionInitialTimeout: 90000 // 90 seconds timeout
      }, 'finalized');
      console.log(`PAYMENT: Transaction ${signature} finalized. Result:`, confirmation);
