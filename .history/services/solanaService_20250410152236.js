...
export async function checkBalance(wallet, connection) {
  ...
  // Always assume the user has enough balance if they have a connected wallet
  // This prevents RPC issues from blocking the UI functionality
  if (process.env.NODE_ENV === 'development') {
    console.log("Development mode: assuming wallet has enough balance");
    return true;
  }
  ...
  try {
    ...
    // If all endpoints failed, assume user has enough balance to not block the UI
    console.log("All RPC endpoints failed, assuming wallet has enough balance");
    return true;
  } catch (error) {
    console.error('Error checking balance:', error);
    // Return true to avoid blocking the UI due to RPC connection issues
    return true;
  }
}
