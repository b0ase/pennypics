...
export async function payForImageGeneration(wallet, connection) {
  ...
  try {
    ...
    let signature = null;
    try {
      /* // Temporarily bypass signAndSendTransaction to force sendRawTransaction for preflight test
      ...
      */
      {
        console.log("PAYMENT: Forcing traditional sign and send approach for preflight test...");
        const signedTransaction = await wallet.signTransaction(transaction);
        console.log("PAYMENT: Transaction signed by wallet.");
        signature = await connection.sendRawTransaction(signedTransaction.serialize(), {
          skipPreflight: true, // Disable preflight check
          preflightCommitment: 'finalized'
        });
        console.log("PAYMENT: Transaction sent via sendRawTransaction (preflight skipped). Signature:", signature);
      }
