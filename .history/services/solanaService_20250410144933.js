...
export async function checkBalance(wallet, connection) {
  ...
  try {
    ...
    try {
      ...
      // If we have a 403 or 429 error, try using a fallback endpoint
      if (rpcError.message && (rpcError.message.includes('403') || rpcError.message.includes('429'))) {
        console.log("Attempting to use fallback RPC endpoint");
        
        // Let's create a fallback connection
        const fallbackEndpoint = 'https://solana-api.projectserum.com';
        const fallbackConnection = new Connection(fallbackEndpoint, 'confirmed');
        ...
        try {
          ...
          console.error('Fallback RPC endpoint error:', fallbackError);
          throw new Error('Could not connect to Solana network. Please try again later.');
        }
      }
