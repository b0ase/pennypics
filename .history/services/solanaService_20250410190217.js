...
export async function payForImageGeneration(wallet, connection) {
  ...
  try {
    ...
    try {
      ...
      // Use confirmTransaction with the strategy, commitment, and timeout
      const confirmation = await connection.confirmTransaction(strategy, 'finalized');
      // Note: The separate timeout parameter might not be needed when passing the full strategy object.
      // Let's rely on the connection's default or the one set in WalletContextProvider.
