...
export async function payForImageGeneration(wallet, connection) {
  ...
  try {
    ...
    try {
      ...
      console.error('Error getting blockhash, trying fallback:', blockHashError);
      ...
      // Update the connection to use for the rest of the transaction
...
export async function checkBalance(wallet, connection) {
  ...
  try {
    ...
    try {
      // First try getting the latest blockhash to check RPC connection
      ...
      console.error('RPC endpoint error:', rpcError);
      
      // Try multiple fallback endpoints sequentially
      ...
      for (const endpoint of fallbackEndpoints) {
        try {
          console.log(`Attempting fallback RPC endpoint: ${endpoint}`);
          const fallbackConnection = new Connection(endpoint, 'confirmed');
          
          // Test if this endpoint works
          const { blockhash } = await fallbackConnection.getLatestBlockhash('confirmed');
          console.log(`Fallback endpoint working, got blockhash: ${blockhash.substring(0, 10)}...`);
          
          // Get the balance using this endpoint
          const balance = await fallbackConnection.getBalance(wallet.publicKey, 'confirmed');
          const requiredLamports = IMAGE_COST * LAMPORTS_PER_SOL;
          const hasEnough = balance >= requiredLamports;
          
          console.log(`[FALLBACK] Wallet balance: ${balance/LAMPORTS_PER_SOL} SOL (${balance} lamports)`);
          console.log(`[FALLBACK] Required balance: ${IMAGE_COST} SOL (${requiredLamports} lamports)`);
          console.log(`[FALLBACK] Has enough: ${hasEnough}`);
          
          // If this endpoint worked, save it to localStorage for future use
          if (typeof localStorage !== 'undefined') {
            localStorage.setItem('solana-rpc-url', endpoint);
            console.log(`Saved working RPC endpoint to localStorage: ${endpoint}`);
          }
          
          return hasEnough;
        } catch (fallbackError) {
          console.error(`Fallback RPC endpoint error for ${endpoint}:`, fallbackError);
          // Continue to the next fallback endpoint
        }
      }
      
      // If we've tried all fallbacks and none worked, throw a clear error
      throw new Error('Could not connect to any Solana RPC endpoint. Please try changing the RPC provider or try again later.');
    }
  } catch (error) {
