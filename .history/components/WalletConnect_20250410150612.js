...
const WalletContextProvider = ({ children }) => {
  ...
  useEffect(() => {
      ...
      if (!isHealthy) {
        ...
        for (const fbEndpoint of fallbackEndpoints) {
          ...
          if (isWorking) {
            ...
            // Instead of reloading the page, just update the endpoint
            setRpcHealth(true);
            // The app will use this endpoint on next connection request
            break;
          }
        }
      }
    ...
    const intervalId = setInterval(checkEndpointHealth, 30000); // Check every 30 seconds
  ...
  return (
    <>
      {/* RPC health alert removed to prevent showing users connection issues */}
