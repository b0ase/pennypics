...
const PRIORITY_FEE_MICROLAMPORTS = 10_000; // 0.00001 SOL
...
export const sendPaymentTransaction = async (connection, wallet) => {
  try {
    ...
    try {
      ...
      try {
        // First try the preferred confirmation method
        try {
          ...
          // Wait a moment for the transaction to potentially complete
          ...
          if (status?.value?.confirmationStatus === 'confirmed' || 
            ...
            // Transaction status is unknown, likely expired
            throw new Error("Block height exceeded or transaction not found");
          }
        }
      } catch (finalError) {
