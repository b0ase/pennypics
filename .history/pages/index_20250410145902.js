...
export default function Home() {
  ...
  useEffect(() => {
    ...
    checkUserBalance();
  }, [wallet, wallet.publicKey, connection, isMounted]);
  ...
  const handlePaymentAndGenerate = async () => {
    ...
    try {
      ...
      // For development/testing - allow skipping payment
      if (process.env.NODE_ENV === 'development' && process.env.NEXT_PUBLIC_SKIP_PAYMENT === 'true') {
        ...
        setSuccess(`Development mode: Payment of ${formatSol(IMAGE_COST)} skipped`);
        await generateImage();
        return;
      }
      ...
      try {
        ...
        // Check if this is an RPC connectivity error
        if (balanceError.message.includes('network') || 
            balanceError.message.includes('403') || 
            balanceError.message.includes('429') ||
            balanceError.message.includes('blockhash')) {
          throw new Error(`Connection to Solana network failed. Please try again later or refresh the page. Error: ${balanceError.message}`);
        }
        
        // Pass along the original error if it's not an RPC error
        throw balanceError;
      }
      ...
      setSuccess(`Payment of ${formatSol(IMAGE_COST)} successful! 
                 Recipient: ${recipientAddressInfo?.address || 'Payment address'}
                 Signature: ${paymentResult.signature.slice(0, 8)}...`);
  ...
  return (
    <div style={{ 
      ...
      <main style={{ 
        ...
        {/* Mainnet Environment Banner - hidden as requested */}
        {false && process.env.NEXT_PUBLIC_SOLANA_RPC_URL?.includes('mainnet') && (
          <div style={{
            ...
            <span>
              <strong>Live Mode:</strong> Using Solana Mainnet. Real transactions will be processed with actual SOL.
            </span>
          </div>
        )}
        ...
        {process.env.NEXT_PUBLIC_SOLANA_RPC_URL?.includes('devnet') && (
          <div style={{
            ...
            <span>
              <strong>Test Mode:</strong> Using Solana Devnet. Real transactions will be processed, but with test SOL.
            </span>
          </div>
        )}
        ...
        <div style={{
          ...
          {/* Payment address verification - hidden as requested */}
          {false && process.env.NODE_ENV === 'development' && recipientAddressInfo && (
            <div style={{
              marginTop: '1rem',
              padding: '0.75rem',
              backgroundColor: recipientAddressInfo.valid ? 'var(--success-bg)' : 'var(--error-bg)',
              color: recipientAddressInfo.valid ? 'var(--success-text)' : 'var(--error-text)',
              borderRadius: '6px',
              fontSize: '0.85rem',
              textAlign: 'left'
            }}>
              <p style={{ margin: '0 0 0.5rem 0', fontWeight: '600' }}>
                {recipientAddressInfo.valid ? '✅ Payment Address Verified' : '⚠️ Payment Address Issue'}
              </p>
              <p style={{ margin: '0', wordBreak: 'break-all' }}>
                Sending payments to: {recipientAddressInfo.address}
              </p>
            </div>
          )}
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            <button 
              ...
              disabled={loading || isProcessingPayment || !prompt.trim()}
              style={{
                backgroundColor: (loading || isProcessingPayment || !prompt.trim()) 
                  ? 'var(--text-secondary)' 
                  : 'var(--accent-color)',
                color: 'white',
                border: 'none',
                padding: '0.9rem 2rem',
                borderRadius: '8px',
                cursor: (loading || isProcessingPayment || !prompt.trim()) 
                  ? 'not-allowed' 
                  : 'pointer',
                fontWeight: '600',
                fontSize: '1rem',
            ...
            >
              {loading || isProcessingPayment ? (
                ...
                <>
                  ...
                  {wallet.publicKey ? `Pay ${formatSol(IMAGE_COST)} & Generate` : 'Connect Wallet to Generate'}
                </>
              )}
            </button>
            {!wallet.publicKey && (
            ...
            {wallet.publicKey && (
              <div style={{ 
                marginTop: '0.75rem', 
                fontSize: '0.9rem',
                color: hasEnoughBalance ? 'var(--success-text)' : 'var(--error-text)',
                fontWeight: '500',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                ...
                {hasEnoughBalance ? (
                  <>
                    ...
                    Wallet has sufficient balance ({formatSol(IMAGE_COST)})
                  </>
                ) : (
                  <>
                    <svg style={{ width: '1rem', height: '1rem' }} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    Insufficient wallet balance. Need at least {formatSol(IMAGE_COST)}.
                  </>
                )}
              </div>
            )}
        ...
        {success && (
          <div style={{ 
            ...
            <svg style={{ width: '1.5rem', height: '1.5rem', flexShrink: 0, marginTop: '3px' }} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <div>
              <p style={{ fontWeight: '600', margin: 0 }}>Success</p>
              <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.9rem', whiteSpace: 'pre-line' }}>{success}</p>
              {transactionLink && (
                <a 
                ...
                >
                  <svg style={{ width: '0.85rem', height: '0.85rem' }} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
