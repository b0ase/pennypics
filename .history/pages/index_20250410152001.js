  ...
  useEffect(() => {
    ...
    const checkUserBalance = async () => {
      if (wallet && wallet.publicKey) {
        try {
          // Always set hasEnoughBalance to true when wallet is connected
          // This prevents the button from being disabled due to RPC issues
          setHasEnoughBalance(true);
          
          // Try to check the actual balance, but don't block UI functionality
          try {
            const hasBalance = await checkBalance(wallet, connection);
            // Only update UI if balance is insufficient
            if (!hasBalance) {
            ...
            // Don't show error in the UI, just log it and assume there's enough balance
          }
        } catch (err) {
          console.error("Wallet connection error:", err);
          // Still allow functionality even if there's an error
          setHasEnoughBalance(true);
        }
      } else {
        setHasEnoughBalance(false);
  ...
  const handlePaymentAndGenerate = async () => {
    ...
    const paymentTimeout = setTimeout(() => {
      ...
      if (isProcessingPayment) {
        ...
        setSuccess("Starting image generation...");
        // If payment times out, still generate the image
        generateImage();
      }
    ...
    try {
      ...
      try {
        ...
        setSuccess(`Payment sent. Starting image generation...`);
        ...
        // If in development or if error includes network issues, proceed with generation
        setHadNetworkIssues(true);
        console.log("Payment failed but proceeding with image generation");
        setSuccess("Payment may have failed. Proceeding with image generation...");
        await generateImage();
      }
    } catch (err) {
      clearTimeout(paymentTimeout);
      console.error("Payment processing error:", err);
      // Don't block generation even if there was an error
      console.log("Payment error but still generating image");
      setSuccess("Starting image generation...");
      await generateImage();
    } finally {
      clearTimeout(paymentTimeout);
