...
export default function Home() {
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      // Ensure token costs are valid numbers
      const solCost = parseFloat(TOKEN_COST_SOL) * 10;
      if (isNaN(solCost) || solCost <= 0) {
        throw new Error(`Invalid token cost calculation: ${TOKEN_COST_SOL} * 10`);
      }
      
      // Check SOL balance before attempting purchase
      const hasEnoughSOL = await checkBalance(wallet, connection, solCost);
      if (!hasEnoughSOL) {
        throw new Error(`Insufficient SOL balance. You need ${solCost} SOL to purchase 10 credits.`);
      }
      ...
      setSuccess(`Successfully purchased 10 PENNY tokens! Your new balance: ${newCredits} tokens.`);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      try {
        ...
        // Create image URLs with proper data URLs - simplify this logic
        const newImages = data.images.map(imageBase64 => {
          if (!imageBase64) {
        ...
        console.log("GENERATE_IMAGE: First image URL preview:", newImages[0].substring(0, 50) + "...");
      ...
      <main style={{ 
        ...
        <div style={{
          ...
          <p style={{
            ...
            You currently have <strong>{imageCredits} PENNY</strong> tokens. Buy more tokens to generate images!
          </p>
          ...
          >
            {isBuyingCredits ? (
              ...
              <>
                ...
                Buy {TOKENS_PER_PURCHASE} PENNY for {formatSol(TOKEN_COST_SOL * TOKENS_PER_PURCHASE)}
              </>
            )}
          </button>
        </div>
