...
import { payForImageCredits, checkBalance, IMAGE_COST, formatSol, LAMPORTS_PER_SOL } from '../services/solanaService';
...
export default function Home() {
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      // Optional: Check balance before attempting purchase (can be skipped)
      // const hasEnoughForCredits = await checkBalance(wallet, connection, CREDIT_PURCHASE_COST_SOL);
      // if (!hasEnoughForCredits) { ... }
      
      console.log("BUY_CREDITS: Calling payForImageCredits...");
      // We might need a dedicated function if the amount/recipient changes, 
      // or pass the amount to the existing function.
      // Assuming payForImageCredits exists or payForImageGeneration is adapted:
      const paymentResult = await payForImageCredits(wallet, connection, CREDIT_PURCHASE_COST_SOL);
      
      console.log("BUY_CREDITS: Payment successful:", paymentResult.signature);
      const newCredits = imageCredits + CREDITS_PER_PURCHASE;
      setImageCredits(newCredits);
      setSuccess(`Successfully purchased ${CREDITS_PER_PURCHASE} credits! Total: ${newCredits}. Sig: ${paymentResult.signature.slice(0,8)}...`);
