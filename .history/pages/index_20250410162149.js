...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const checkUserBalance = async () => {
      if (wallet && wallet.publicKey) {
        try {
          ...
          setHadNetworkIssues(false); // Clear network issues flag on success
          
          if (!hasBalance) {
            setError(`Insufficient balance. You need at least ${formatSol(IMAGE_COST)} SOL to generate an image.`);
          }
        } catch (balanceError) {
          console.error("Error checking balance:", balanceError);
          setHasEnoughBalance(false);
          setError(`Network error: Unable to check wallet balance. Please try refreshing the page or switching to a different RPC endpoint.`);
    ...
    const intervalId = setInterval(checkUserBalance, 10000);
    ...
    <div>
          ...
          <div style={{ textAlign: 'center' }}>
            <button 
              ...
              disabled={loading || isProcessingPayment || !prompt.trim() || !hasEnoughBalance || !wallet.publicKey}
              style={{
                backgroundColor: (loading || isProcessingPayment || !prompt.trim() || !hasEnoughBalance || !wallet.publicKey) 
                  ? 'var(--text-secondary)' 
                  : 'var(--accent-color)',
                color: 'white',
                border: 'none',
                padding: '0.9rem 2rem',
                borderRadius: '8px',
                cursor: (loading || isProcessingPayment || !prompt.trim() || !hasEnoughBalance || !wallet.publicKey) 
                  ? 'not-allowed' 
                  : 'pointer',
                fontWeight: '600',
                fontSize: '1rem',
            ...
            >
              {loading || isProcessingPayment ? (
                ...
                <>
                  ...
                  {wallet.publicKey ? 
                    (hasEnoughBalance ? 'Generate Image' : `Insufficient Balance (${formatSol(IMAGE_COST)} SOL required)`) 
                    : 'Connect Wallet to Generate'}
                </>
              )}
            </button>
            ...
            {wallet.publicKey && !hasEnoughBalance && (
              <p style={{
                fontSize: '0.9rem',
                ...
                Please add {formatSol(IMAGE_COST)} SOL to your wallet to generate images
              </p>
            )}
