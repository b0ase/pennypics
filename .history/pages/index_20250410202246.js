...
export default function Home() {
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      // Use fixed values
      const solCost = 0.01; // Fixed value: 0.01 SOL for 10 tokens
      
      // Check SOL balance before attempting purchase
      const hasEnoughSOL = await checkBalance(wallet, connection, solCost);
      if (!hasEnoughSOL) {
        throw new Error(`Insufficient SOL balance. You need 0.01 SOL to purchase 10 credits.`);
      }
      ...
      // Update credits display - add exactly 10 tokens
      const currentCredits = parseInt(localStorage.getItem(CREDITS_STORAGE_KEY) || '0', 10);
      ...
      setSuccess(`Successfully purchased 10 credits! Your new balance: ${newCredits} credits.`);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      // Check if user has enough PENNY tokens and spend them
      console.log(`GENERATE_IMAGE: Attempting to spend ${IMAGE_GENERATION_COST_CREDITS} PENNY token(s). Current balance: ${imageCredits}`);
      
      if (imageCredits < IMAGE_GENERATION_COST_CREDITS) {
        throw new Error(`Insufficient PENNY tokens. You need ${IMAGE_GENERATION_COST_CREDITS} token(s). Please buy more.`);
      }
      
      // Spend the tokens (deducts from localStorage)
      const spendResult = spendTokensForImage();
      console.log(`GENERATE_IMAGE: Spent tokens. Remaining: ${spendResult.remainingTokens}`);
      setImageCredits(spendResult.remainingTokens);
      ...
      try {
        ...
        console.log("GENERATE_IMAGE: Full API response:", data);
        
        let newImages = [];
        
        if (!data.images || !Array.isArray(data.images) || data.images.length === 0) {
          console.warn('No images in API response, using emergency fallback image');
          // Emergency fallback image - a simple generated SVG
          const fallbackImage = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
            `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
              <rect width="512" height="512" fill="#f0f0f0"/>
              <text x="256" y="256" font-family="Arial" font-size="20" text-anchor="middle" dominant-baseline="middle" fill="#333">
                Generated image based on prompt:
              </text>
              <text x="256" y="286" font-family="Arial" font-size="16" text-anchor="middle" dominant-baseline="middle" fill="#666" font-style="italic">
                "${prompt.substring(0, 50)}${prompt.length > 50 ? '...' : ''}"
              </text>
            </svg>`
          )}`;
          newImages = [fallbackImage];
        } else {
          // Create image URLs with proper data URLs
          newImages = data.images.map(imageBase64 => {
            if (!imageBase64) {
              console.error("Invalid image data: null or undefined");
              return null;
            }
            
            // Already has data:image/png;base64 prefix
        ...
        if (newImages.length === 0) {
          throw new Error('Images could not be processed correctly');
        }
        
        console.log("GENERATE_IMAGE: Created image URLs. Total images:", newImages.length);
        ...
        // Add credit back if image generation fails
        console.error("GENERATE_IMAGE: API error:", apiError);
        setImageCredits(prev => prev + 1); // Add credit back
        throw apiError;
      }
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{
              ...
              Each image generation costs 1 PENNY token
            </p>
          </div>
          <p style={{
            margin: '0 auto 1rem',
            ...
            You currently have <strong>{imageCredits} credits</strong> in your account. These are local credits backed by your SOL payment.
          </p>
          ...
          <button
            ...
            style={{
              backgroundColor: isBuyingCredits || !wallet.publicKey ? 'var(--text-secondary)' : 'var(--accent-color)',
              color: 'white',
              border: 'none',
              padding: '0.5rem 1.5rem',
              borderRadius: '8px',
              cursor: isBuyingCredits || !wallet.publicKey ? 'not-allowed' : 'pointer',
              fontWeight: '500',
              fontSize: '0.9rem',
              display: 'inline-flex',
              alignItems: 'center',
          ...
          >
            {isBuyingCredits ? (
              <><span className="spinner" style={spinnerStyle}></span> Buying...</>
            ) : (
              <>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            >
              {loading ? (
                ...
                'Insufficient PENNY Tokens'
              ) : (
                <>{/* SVG Icon */} Generate Image (1 PENNY)</>
              )}
            </button>
                ...
                <div style={{ 
                  ...
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.target.onerror = null;
                      e.target.src = 'data:image/svg+xml;base64,';
                    }}
                  />
                  <div style={{ padding: '0.5rem', textAlign: 'center', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    If you can't see the image, try the Download button above to save it.
                  </div>
                </div>
              )}
            </div>
