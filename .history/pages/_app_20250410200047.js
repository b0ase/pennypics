...
function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    setMounted(true);
    const savedMode = safeLocalStorage.get(STORAGE_KEYS.DARK_MODE);
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)');
    setDarkMode(savedMode !== null ? savedMode : prefersDark.matches);

    const handleChange = (e) => {
      if (safeLocalStorage.get(STORAGE_KEYS.DARK_MODE) === null) {
        setDarkMode(e.matches);
      }
    };
    prefersDark.addEventListener('change', handleChange);

    const savedHistory = safeLocalStorage.get(STORAGE_KEYS.IMAGE_HISTORY, []);
    setImageHistory(savedHistory);

    // Add error recovery - if anything crashes, user can still access the test page
    try {
      window.addEventListener('error', (event) => {
        console.error('Global error caught:', event.error);
        const isOnTestPage = window.location.pathname === '/test';
        if (!isOnTestPage) {
          console.log('Redirecting to safe test page due to error');
  ...
  useEffect(() => {
    if (mounted && darkMode !== undefined) {
      safeLocalStorage.set(STORAGE_KEYS.DARK_MODE, darkMode);
  ...
  useEffect(() => {
    if (mounted && imageHistory.length > 0) {
      safeLocalStorage.set(STORAGE_KEYS.IMAGE_HISTORY, imageHistory);
    }
  }, [imageHistory, mounted]);

  const addToImageHistory = (imageData) => {
    setImageHistory(prev => {
      const newHistory = [imageData, ...prev].slice(0, MAX_HISTORY_SIZE);
      safeLocalStorage.set(STORAGE_KEYS.IMAGE_HISTORY, newHistory);
      return newHistory;
    });
  };
