...
export default function Home() {
  ...
  const handlePaymentAndGenerate = async () => {
    ...
    try {
      ...
      if (!hasBalance) {
        // Add dev mode override option
        if (process.env.NODE_ENV === 'development' && process.env.NEXT_PUBLIC_FORCE_GENERATE === 'true') {
          console.log("Development mode: Forcing image generation despite insufficient balance");
          await generateImage();
          return;
        }
      ...
      <main style={{ 
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            {/* Development mode skip payment button - only shown in development */}
            {process.env.NODE_ENV === 'development' && (
              <button 
                onClick={async () => {
                  setError(null);
                  setSuccess('Development mode: Skipping payment and balance check');
                  await generateImage();
                }}
                style={{
                  marginTop: '1rem',
