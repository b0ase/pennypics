  ...
  const handlePaymentAndGenerate = async () => {
    ...
    try {
      ...
      try {
        ...
        if (paymentError.message.includes('RPC connection error') || 
            paymentError.message.includes('403') || 
            paymentError.message.includes('blockhash')) {
          setError(`Payment failed: RPC connection error. Please try using a different wallet or refresh the page.`);
        } else {
