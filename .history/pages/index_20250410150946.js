...
export default function Home() {
  ...
  const handlePaymentAndGenerate = async () => {
    ...
    // Track if we've had network issues
    let hadNetworkIssues = false;
    ...
    try {
      ...
      try {
        ...
        if (paymentError.message && (
            ...
            paymentError.message.includes('failed to fetch') ||
            paymentError.message.includes('blockhash') ||
            paymentError.message.includes('RPC')
          )) {
          hadNetworkIssues = true;
          throw new Error("Connection issue detected. Please try again or use a different network.");
        } else {
    ...
    // Return whether we had network issues (for the component to know whether to show the network selector)
    return hadNetworkIssues;
  };
      ...
      <main style={{ 
        ...
        <div style={{
          ...
          <p style={{
            ...
            If you have connection issues, the app will automatically try alternative Solana networks
          </p>
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            {wallet.publicKey && error && error.includes("connection") && (
              <div style={{ 
                marginTop: '0.75rem',
                display: 'flex',
                justifyContent: 'center'
              }}>
                <SolanaRpcSelector onRpcChange={() => {}} />
              </div>
            )}
