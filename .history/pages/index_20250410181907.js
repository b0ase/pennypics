...
export default function Home() {
  ...
  useEffect(() => {
    ...
    checkUserBalance();
  }, [wallet, connection]); // Remove wallet.publicKey from dependencies
  ...
  const handlePaymentAndGenerate = async () => {
    ...
    try {
      if (!wallet || !wallet.publicKey) {
        ...
        console.error("HANDLE_PAYMENT: Wallet not connected.");
        setIsProcessingPayment(false);
        return;
      }

      console.log(`HANDLE_PAYMENT: Wallet connected: ${wallet.publicKey.toString()}`);

      // Check if user has enough balance
      let hasBalance = false;
      try {
        console.log("HANDLE_PAYMENT: Calling checkBalance...");
        hasBalance = await checkBalance(wallet, connection);
        console.log(`HANDLE_PAYMENT: checkBalance returned: ${hasBalance}`);
      } catch (balanceError) {
        console.error("HANDLE_PAYMENT: Error calling checkBalance:", balanceError);
        setError(balanceError.message || 'Failed to check wallet balance.');
        setIsProcessingPayment(false);
        return;
      }
      
      if (!hasBalance) {
        console.error(`HANDLE_PAYMENT: Insufficient balance reported. Needed ${formatSol(IMAGE_COST)}`);
        setError(`Insufficient balance. You need at least ${formatSol(IMAGE_COST)} to generate an image.`);
        setIsProcessingPayment(false);
        return;
      }

      console.log("HANDLE_PAYMENT: Balance check passed. Proceeding to payment...");
      ...
      try {
        ...
        setSuccess(`Payment successful! Generating your image...`);
        ...
        if (paymentError.message.includes('Failed to get recent blockhash')) {
          setError(`Payment failed: Could not connect reliably to Solana network. Please try again later.`);
        } else if (paymentError.message.includes('signing or sending')) {
          setError(`Payment failed: Wallet error during transaction signing. Please try again.`);
        } else {
  ...
  const generateImage = async () => {
    ...
    try {
      // Show a spinner for at least 1 second to avoid flickering UI
